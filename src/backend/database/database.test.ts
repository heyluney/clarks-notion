...
const databaseTest = test.extend<{ database: Database }>({
    database: async ({ }, use) => {
        const database: Database = {
            1: createTestComponent(ComponentEnum.Page, 1, 0, [2], {}),
            2: createTestComponent(ComponentEnum.Emoji, 2, 1, [], {})
        };
...
describe("component insertion", () => {
    databaseTest('component can be inserted into to database', ({ database }) => {
        const journal : Component = createTestComponent(ComponentEnum.Journal, 1, 0, [], {});
